...
const fetchTrekkers = async () => {
  ...
  errorMsg.value = ''
  try {
    const res = await api.get(`/api/staff/treks/${props.trek.id}/users`)
    registeredUsers.value = res.registered_users || []
    totalRegistered.value = res.total_registered || 0
  } catch (err) {
    if (err.status === 403) {
      errorMsg.value = 'Forbidden. You are not assigned to manage this trek.'
    } else {
      errorMsg.value = err.message || 'Failed to fetch registered trekkers.'
    }
  } finally {
    loading.value = false
  }
      ...
      <div class="modal-content bg-white border-0 shadow-lg rounded-4 overflow-hidden">
        ...
        <div class="modal-header bg-dark-slate text-white py-3 px-4 d-flex align-items-center justify-content-between">
          <div>
            <h5 class="modal-title fs-6 fw-bold mb-0 d-flex align-items-center">
              <i class="bi bi-people-fill me-2 text-emerald"></i>
              Registered Trekkers Roster
            </h5>
            <small class="text-white-50" v-if="trek">
              {{ trek.trek_name }} &bull; <i class="bi bi-geo-alt me-0.5"></i>{{ trek.location }}
            </small>
          </div>
        ...
        <div class="modal-body p-4">
          ...
          <div class="d-flex flex-column flex-sm-row justify-content-between align-items-sm-center gap-3 mb-4">
            ...
            <div class="badge bg-emerald-subtle text-emerald border border-emerald fs-7 py-2 px-3 rounded-pill text-nowrap">
              <i class="bi bi-person-check-fill me-1.5"></i>Total Registered: <strong>{{ totalRegistered }}</strong>
            </div>
          </div>
          ...
          <div v-if="loading" class="text-center py-5">
            <div class="spinner-border text-emerald" role="status">
              <span class="visually-hidden">Loading trekkers...</span>
            </div>
            <p class="text-muted small mt-2">Loading trekker roster...</p>
          </div>
          ...
          <div v-else-if="!loading" class="text-center py-5 bg-light rounded-4 border border-dashed">
            ...
            <p class="text-muted small mb-0">
              {{ searchQuery ? 'No participants match your search criteria.' : 'No registered trekkers for this trek yet.' }}
            </p>
          </div>
        </div>
        ...
        <div class="modal-footer bg-light border-0 py-3 px-4 d-flex justify-content-between">
          <small class="text-muted fs-8">Assigned Staff Access Only</small>
